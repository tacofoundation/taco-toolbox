...
class TestSampleIdValidation:
    ...
    def test_rejects_invalid_ids(self, make_file, invalid_id, reason):
        ...
        assert (
            invalid_id in str(exc_info.value) or reason in str(exc_info.value).lower()
        )
...
class TestSampleExtendWith:
    ...
    def test_extends_with_single_row_arrow_table(
        self, make_sample, single_row_arrow_table
    ):
        sample = make_sample()
